    ...
    <section class="bg-gray-50 dark:bg-gray-950 min-h-screen flex items-center justify-center">
        <div class="bg-gray-100 dark:bg-gray-800 flex rounded shadow-lg max-w-3xl p-5 items-center" data-aos="fade" data-aos-duration="700">
            ...
            <div class="md:w-1/2 px-10">
                ...
                <form method="POST" action="{{ route('post-seller') }}" class="flex flex-col gap-4" enctype="multipart/form-data">
                    @csrf
                    @method("POST")
                    {{-- NAME --}}
                    <div>
                    ...
                    {{-- PHONE --}}
                    <div>
                        <input class="p-2 rounded border bg-white w-full @error('phone') border-red-500 @enderror" 
                               type="text" 
                               name="phone" 
                               placeholder="Phone" 
                               value="{{ old('phone') }}" 
                    ...
                    {{-- ktm --}}
                    <div>
                        <label for="ktm" class="cursor-pointer p-2 rounded border bg-white w-full block text-gray-500 text-start hover:bg-gray-100 @error('ktm') border-red-500 @enderror">
                            Upload foto KTM
                        </label>
                        <input id="ktm" name="ktm" type="file" class="hidden" required>
                        @error('ktm')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
