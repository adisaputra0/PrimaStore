...
<head>
   ...
   {{-- Toast --}}
   <link rel="stylesheet" href="{{ asset('toast-notification/style.css') }}">
   {{-- LightBox --}}
   ...
   <div class="px-3 py-3 lg:px-5 lg:pl-3">
      ...
      <div class="flex items-center justify-start rtl:justify-end">
         ...
         <a href="{{ route('dashboard') }}" class="flex ms-2 md:me-24">
            ...
            <span class="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap dark:text-white">PrimaStore</span>
         </a>
      </div>
      <div class="flex items-center">
            <div class="flex items-center ms-3">
            <div class="flex justify-center items-center">
               <button id="theme-toggle" type="button" class="me-3 cursor-pointer text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5">
               ...
               <button type="button" class=" cursor-pointer flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" aria-expanded="false" data-dropdown-toggle="dropdown-user">
                  <span class="sr-only">Open user menu</span>
                        @if(auth()->user()->image)
                           <img class="w-8 h-8 rounded-full" src="{{ asset('images/photos/' . auth()->user()->image) }}" alt="user photo">
                        @else
            ...
            <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-sm shadow-sm dark:bg-gray-700 dark:divide-gray-600" id="dropdown-user">
               ...
               <ul class="py-1" role="none">
                  ...
                  <li>
                  <a data-modal-target="edit-profile" data-modal-toggle="edit-profile" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Settings</a>
                  </li>
                  {{-- <li>
               ...
               @if(auth()->user()->role == "penjual" || auth()->user()->role == "pembeli")
                  <div class="px-4 py-3" role="none">
                     <p class="text-sm font-medium text-gray-900 truncate dark:text-gray-300" role="none">
                        Koin: {{ auth()->user()->wallet->balance ?? 0 }}PK
                     </p>
                  </div>
               @endif
            </div>
...
{{-- Edit Profile --}}
<div id="edit-profile" data-modal-placement="bottom-center" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[100%] max-h-full bg-[rgba(0,0,0,0.5)]">
   <div class="relative w-full max-w-md max-h-full p-4">
         ...
         <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div id="modal-body-edit-profile">
               <!-- Modal body -->
               <form method="POST" action="{{ route('update-profile') }}" enctype="multipart/form-data" class="p-4 md:p-5 mb-5">
                  @csrf
                  @method("POST")
                  <div class="flex flex-col gap-5 justify-center items-center">
                     <div class="relative">
                        @if(auth()->user()->image)
                              {!! '<img src="' . asset('images/photos/' . auth()->user()->image) . '" alt="Profil User" width="200px" class="m-0 h-full rounded object-cover">' !!}
                        @else
                              {!! '<img src="' . asset('images/user.jpg') . '" alt="Profil User" width="200px" class="m-0 h-full rounded object-cover">' !!}
                        @endif
                        <input type="file" name="image" id="image" class="absolute top-0 left-0 h-full w-full opacity-0">
                     </div>
                     <div class="grid gap-4 mb-4 grid-cols-2">
                           <div>
                              <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                              <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-800 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type name" required="" value="{{ auth()->user()->name }}">
                           </div>
                           <div>
                              <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                              <input type="text" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-800 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" hidden placeholder="Type email" required="" value="{{ auth()->user()->email }}">
                           ...
                           <div>
                              ...
                              <input type="text" name="phone" id="phone" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-800 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type phone" value="{{ auth()->user()->phone? auth()->user()->phone:'-' }}">
                           </div>
                           <div>
                              <label for="bio" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Bio</label>
                              <input type="text" name="bio" id="bio" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-800 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type bio" value="{{ auth()->user()->bio }}">
                           </div>
                     </div>
                  </div>
...
{{-- Toast --}}
<script src="{{ asset('toast-notification/script.js') }}"></script>
@if(session('message'))
<script>
   createToast("{{ session('message.icon') }}", "{{ session('message.text') }}");
</script>
@endif
...
<script>
   $(document).ready(function () {
      // Pastikan elemen #myTable ada sebelum menginisialisasi DataTable
      if ($('#myTable').length) {
         $('#myTable').DataTable({
            "responsive": true,
